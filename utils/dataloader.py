...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            shuffle(lines)
            image, box = self.get_random_data_with_Mosaic(lines, self.input_size)

            if self.mixup and self.rand() < self.mixup_prob:
                lines = sample(self.annotation_lines, 1)
                image_2, box_2 = self.get_random_data(
                    lines[0], self.input_size, random=self.train
                )
                image, box = self.get_random_data_with_MixUp(image, box, image_2, box_2)
        else:
            image, box = self.get_random_data(
                self.annotation_lines[index], self.input_size, random=self.train
            )
        ...
        if len(box) != 0:
            box[:, [0, 2]] = box[:, [0, 2]] / self.input_size
            box[:, [1, 3]] = box[:, [1, 3]] / self.input_size
    ...
    def get_random_data_with_Mosaic(
        self, annotation_line, input_size, jitter=0.3, hue=0.1, sat=0.7, val=0.4
    ):
        h, w = input_size, input_size
