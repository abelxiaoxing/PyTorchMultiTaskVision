...
class YOLOLoss(nn.Module):
    def __init__(
        ...
        self.box_ratio = 0.05
        self.obj_ratio = 5 * (input_size**2) / (224**2)
    ...
    def forward(self, l, input, targets=None):
        ...
        # -----------------------------------------------------------------------#
        stride_h = self.input_size / in_h
        stride_w = self.input_size / in_w
        # -------------------------------------------------#
        ...
        # -----------------------------------------------#
        prediction = (
            input.view(bs, len(self.anchors_mask[l]), self.bbox_attrs, in_h, in_w)
            .permute(0, 1, 3, 4, 2)
            .contiguous()
        )
        ...
        if n != 0:
            ...
            loss_cls = torch.mean(
                self.BCELoss(pred_cls[obj_mask], y_true[..., 5:][obj_mask])
            )
        ...
        if self.focal_loss:
            ...
            loss_conf = (
                torch.mean(
                    (
                        self.BCELoss(conf, obj_mask.type_as(conf))
                        * pos_neg_ratio
                        * hard_easy_ratio
            ...
            loss_conf = torch.mean(
                self.BCELoss(conf, obj_mask.type_as(conf))[noobj_mask.bool() | obj_mask]
            )
    ...
    def get_ignore(
        self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w, noobj_mask
    ):
        # -----------------------------------------------------#
        #   计算一共有多少张图片
        # -----------------------------------------------------#
...
def get_lr_scheduler(
    ...
    def yolox_warm_cos_lr(
        lr, total_iters, warmup_total_iters, warmup_lr_start, no_aug_iter, iters
    ):
